...
function corAtual(e) {
    ...
    //fechar a paleta apos selecionar a cor
...
//cor a partit do seletor de cores
...
function draw(event) {
    ...
    if (is_drawing) {
        ...
        if (tool_select == 'circulo') {
            ...
            context.arc(mpx, mpy, event.clientX - mpx, 0, 2 * Math.PI, false);
...
function salvar() {
    ...
    var imagem = canvas.toDataURL("image/" + formato + "", 1.0).replace("image/" + formato + "", "image/octet-stream");
